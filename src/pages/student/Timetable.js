...
// Default timetable if none saved in DB
...
const Timetable = () => {
  ...
  const isCurrentPeriod = (entry) => {
    ...
    const [startH, startM] = entry.start_time?.split(':') || entry.startTime?.split(':') || ['0', '0'];
    const [endH, endM] = entry.end_time?.split(':') || entry.endTime?.split(':') || ['0', '0'];
            ...
            {todayEntries.map((entry, i) => {
              ...
              return (
                <Card key={i} sx={{
                  ...
                  bgcolor: isBreak ? '#fafafa' : 'white',
                  opacity: isBreak ? 0.7 : 1,
                }}>
                  <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Box>
                        <Typography variant="body2" fontWeight={700} color={isBreak ? 'text.secondary' : getColor(subj)}>
                          {subj}
                        </Typography>
                        {!isBreak && (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {[1,2,3,4,5,6,7].map(period => {
                        const mondayEntry = timetable.find(e => e.day === 'Monday' && (e.period === period || e.period === period));
                        const timeLabel = mondayEntry ? (mondayEntry.start_time || mondayEntry.startTime) : '';
                        return (
                          <TableRow key={period} hover>
                            <TableCell sx={{ bgcolor: '#f8f9fa', fontWeight: 600, fontSize: 12 }}>
                              {timeLabel}
                            </TableCell>
                            {days.map(day => {
