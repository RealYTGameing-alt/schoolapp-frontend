...
const LessonPlans = () => {
  ...
  const handleDelete = async (plan) => {
    ...
    try {
      await api.delete(`/lesson-plans/${plan.id}`);
    } catch (err) {}
    ...
    <Layout menuItems={teacherMenu}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {thisWeekCount} plans submitted this week
          </Typography>
        </Box>
        <Button variant="contained" startIcon={<AddIcon />}
          onClick={() => { setError(''); setCreateDialog(true); }}
          sx={{ borderRadius: 2 }}>
          New Lesson Plan
        </Button>
      </Box>
                  ...
                  <Box sx={{ flexGrow: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5, flexWrap: 'wrap' }}>
                      ...
                      <Chip label={plan.status || 'draft'} size="small"
                        color={plan.status === 'submitted' ? 'success' : 'warning'} />
                    </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5 }}>
                    <Tooltip title="View">
                      <IconButton size="small" color="primary"
                        onClick={() => { setSelectedPlan(plan); setViewDialog(true); }}>
      ...
      <Dialog open={createDialog} onClose={() => setCreateDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <TextField label="Title" fullWidth value={newPlan.title}
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            ...
            <TextField label="Date" type="date" InputLabelProps={{ shrink: true }}
              value={newPlan.date} onChange={e => setNewPlan({...newPlan, date: e.target.value})}
              sx={{ flex: 1, minWidth: 150 }} />
            <TextField label="Duration (mins)" type="number"
              value={newPlan.durationMinutes}
              onChange={e => setNewPlan({...newPlan, durationMinutes: e.target.value})}
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button variant="outlined" onClick={() => {
            setNewPlan({...newPlan, status: 'draft'});
            handleCreate();
          }} disabled={!newPlan.title || saving}>
            Save as Draft
          </Button>
          <Button variant="contained" onClick={() => {
            setNewPlan({...newPlan, status: 'submitted'});
            handleCreate();
          }} disabled={!newPlan.title || saving}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                <Chip label={selectedPlan.subject} size="small"
                  sx={{ bgcolor: (subjectColors[selectedPlan.subject] || '#1a73e8') + '20',
                        color: subjectColors[selectedPlan.subject] || '#1a73e8' }} />
                ...
                <Chip label={selectedPlan.status || 'draft'} size="small"
                  color={selectedPlan.status === 'submitted' ? 'success' : 'warning'} />
              </Box>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          {selectedPlan && (
            <>
              ...
              <TextField label="🎯 Learning Objectives" multiline rows={2} fullWidth
                value={selectedPlan.objectives || ''}
                onChange={e => setSelectedPlan({...selectedPlan, objectives: e.target.value})} />
              <TextField label="📝 Lesson Content" multiline rows={4} fullWidth
                value={selectedPlan.content || ''}
                onChange={e => setSelectedPlan({...selectedPlan, content: e.target.value})} />
              <TextField label="📚 Resources & Materials" multiline rows={2} fullWidth
                value={selectedPlan.resources || ''}
                onChange={e => setSelectedPlan({...selectedPlan, resources: e.target.value})} />
            </>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
